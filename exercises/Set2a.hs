...
palindromify s
  | s == ""   = ""
  | otherwise = if isPalindrome s then s else palindromify (substring 1 (length s - 1) s)
...
safeDiv x y = case y of 0 -> Nothing
                        y -> Just (x `div` y)
...
eitherDiv x y = case y of 0 -> Left (show x ++ "/" ++ show y)
                          y -> Right (x `div` y)
