...
treeSize t =
  case t of
    Empty -> 0
    Node a t1 t2 -> 1 + treeSize t1 + treeSize t2
...
treeMax t =
  case t of
    Empty -> 0
    Node a t1 t2 -> maximum [a, treeMax t1, treeMax t2]
...
allValues condition tree =
  case tree of
    Empty -> True
    Node a t1 t2 ->
      if condition a && allValues condition t1 && allValues condition t2
        then True
        else False
...
isOrdered t =
  case t of
    Empty -> True
    Node a Empty Empty -> True
    Node a t1 Empty ->
      if node_val t1 < Just a
        then True && isOrdered t1
        else False

    Node a Empty t2 ->
      if Just a < node_val t2
        then True && isOrdered t2
        else False

    Node a t1 t2 ->
      if node_val t1 < Just a && Just a < node_val t2
        then True && isOrdered t1 && isOrdered t2
        else False

node_val subtree =
  case subtree of
    Empty -> Nothing
    Node b subtree1 subtree2 -> Just b
...
set [] val tree =
  case tree of
    Empty -> tree
    Node a t1 t2 -> Node val t1 t2

set (step : steps) val tree =
  case tree of
    Empty -> tree
    Node a t1 t2 ->
      case step of
...
search' val Empty path = Nothing
search' val (Node a t1 t2) path
  | a == val    = Just (reverse path)
  | otherwise   =
    case search' val t1 (StepL : path) of
      Just p -> Just p
      Nothing -> search' val t2 (StepR : path)
