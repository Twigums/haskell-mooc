...
appendAll ioref [] = return ()
appendAll ioref (s : ss) =
  do
    modifyIORef ioref (++ s)
    appendAll ioref ss
...
interact' f state =
  do
    ...
    case res of
      (res_bool, res_str, res_state) -> do
        putStrLn res_str

        if res_bool == True
          then interact' f res_state

          else return res_state
