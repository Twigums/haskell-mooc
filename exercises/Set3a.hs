...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a b = case measure a > measure b of True -> a
                                                  False -> b
...
firstHalf x = case even x_len of True -> take (x_len `div` 2) x
                                 False -> take (x_len `div` 2 + 1) x
  where x_len = length x
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (\x -> max >= x) $ powers' k max

powers' :: Int -> Int -> [Int]
powers' k n = [k ^ n | n <- [0 .. n]]
------------------------------------------------------------------------------
...
while check update value = case check value of True -> while check update (update value)
                                               False -> value
...
whileRight check x = case check x of Left left_res -> left_res
                                     Right right_res -> whileRight check right_res
...
joinToLength n s = filter (\x -> length(x) == n) [ word1 ++ word2 | word1 <- s, word2 <- s ]
...
sumRights [] = 0
sumRights list = case head list of Left left_val -> 0 + sumRights (tail list)
                                   Right right_val -> right_val + sumRights (tail list)
...
multiCompose [] target = target
multiCompose fs target = multiCompose (init fs) (last fs $ target)
...
multiApp f gs target = f . multiApp' gs $ target

multiApp' :: [a -> b] -> a -> [b]
multiApp' [] target = []
multiApp' gs target = (head gs) target : multiApp' (tail gs) target
