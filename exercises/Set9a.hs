...
countValid [] = 0
countValid (note : notes) = if (note !! 2 == note !! 4) || (note !! 3 == note !! 5) then 1 + countValid notes else countValid notes
...
repeated [] = Nothing
repeated (item : items) = repeated' items item

repeated' :: Eq a => [a] -> a -> Maybe a
repeated' [] _ = Nothing
repeated' (item : items) last = if item == last then Just item else repeated' items item
...
sumSuccess list = if right_count list > 0 then Right (sumSuccess' list) else Left "no data"
...
right_count [] = 0
right_count (Left x : xs) = right_count xs
right_count (Right x : xs) = 1 + right_count xs
...
open s lock =
  case lock of
    Locked code   -> if s == code then Unlocked code else lock
    _             -> lock
...
instance Eq Text where
  (==) (Text s1) (Text s2) = stripped s1 == stripped s2
    where
      stripped [] = []
      stripped (c : s) =
        case c of
...
compose [] l2 = []
compose (item : l1) l2 =
  case lookup (snd item) l2 of
    Nothing -> compose l1 l2
    Just x  -> (fst item, x) : compose l1 l2
...
permute p l = map (\pair -> snd pair) . sortOn fst $ make_pair p l

make_pair :: [a] -> [b] -> [(a, b)]
make_pair [] [] = []
make_pair (i : is) (j : js) = (i, j) : make_pair is js
