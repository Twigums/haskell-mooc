...
instance Functor List where
  fmap f Empty = Empty
  fmap f (LNode a b) = LNode (f a) (fmap f b)
...
count a f = foldr (\x -> if a == x then (+1) else (+0)) 0 f
...
inBoth a b = foldr (\x -> if elem x a then (++[x]) else (++[])) [] b
...
instance Foldable TwoList where
  foldr f base twolist =
    case twolist of
      TwoEmpty -> base
      TwoNode a b c -> f a (f b (foldr f base c))
