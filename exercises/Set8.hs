...
combine f (Picture p1) (Picture p2) = Picture go
  where go coords = f (p1 coords) (p2 coords)
...
union (Shape f1) (Shape f2) = Shape f
  where f coords = f1 coords || f2 coords

cut :: Shape -> Shape -> Shape
cut (Shape f1) (Shape f2) = Shape f
  where f coords = f1 coords && not (f2 coords)
------------------------------------------------------------------------------
...
paint (Picture pat) (Shape shape) (Picture base) = Picture f
  where f coords | shape coords   = pat coords
                 | otherwise      = base coords
------------------------------------------------------------------------------
...
instance Transform Fill where
  apply (Fill c) (Picture f) = Picture f
    where f coords = c
...
instance Transform Flip where
  apply flip (Picture f) =
    case flip of
      FlipX     -> Picture res
        where res (Coord x y) = f (Coord (-x) y)

      FlipY     -> Picture res
        where res (Coord x y) = f (Coord x (-y))

      FlipXY    -> flipXY (Picture f)
------------------------------------------------------------------------------
...
blendFive :: Color -> Color -> Color -> Color -> Color -> Color
blendFive (Color r1 g1 b1) (Color r2 g2 b2) (Color r3 g3 b3) (Color r4 g4 b4) (Color r5 g5 b5) = Color
  (div (r1 + r2 + r3 + r4 + r5) 5)
  (div (g1 + g2 + g3 + g4 + g5) 5)
  (div (b1 + b2 + b3 + b4 + b5) 5)

instance Transform Blur where
  apply Blur (Picture f) = Picture res
    where res (Coord x y) = blendFive
                              (f (Coord x y))
                              (f (Coord (x + 1) y))
                              (f (Coord (x - 1) y))
                              (f (Coord x (y + 1)))
