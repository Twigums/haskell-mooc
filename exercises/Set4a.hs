...
longest :: Ord a => [[a]] -> [a]
longest xs = last . sortBy (comparing length) . reverse . sort $ xs
...
incrementKey key [] = []
incrementKey key (pair: pairs) =
  if fst pair == key
    then (fst pair, (+1) . snd $ pair) : incrementKey key pairs
    else pair : incrementKey key pairs
...
winner scores player1 player2 =
  ...
  where score1 = Map.findWithDefault 0 player1 scores
...
freqs xs = freqs' xs . Map.fromList . map .nub $ xs
  where map [] = []
        map (val:vals) = (val, 0) : map vals

freqs' [] map = map
freqs' (x:xs) map =
  case Map.lookup x map of
    Nothing -> freqs' xs map
    Just val -> freqs' xs (Map.insert x (val + 1) map)
...
transfer from to amount bank = if Map.member from bank == True && Map.member to bank == True && from_sum >= amount && amount >= 0
  then Map.insert from (from_sum - amount) . Map.insert to (to_sum + amount) $ bank
  else bank
  where
    from_sum = case Map.lookup from bank of
      Nothing -> 0
      Just val -> val
    to_sum = case Map.lookup to bank of
      Nothing -> 0
      Just val -> val
...
maxIndex' :: Ord a => [(i, a)] -> i -> a -> i
maxIndex' [] max_idx max_val = max_idx
maxIndex' (pair:rest) max_idx max_val =
  if snd pair > max_val
    then maxIndex' rest (fst pair) (snd pair)
    else maxIndex' rest max_idx max_val
