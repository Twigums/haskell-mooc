...
checkNumber (s1, s2) =
  if any isDigit s1 || any isDigit s2
    then Nothing
    else Just (s1, s2)
...
winner scores player1 player2 =
  do
    player1_score <- lookup player1 scores
...
balance accountName =
  BankOp $ \(Bank bank) ->
    let bal = (Map.findWithDefault 0 accountName bank)
    in (bal, Bank bank)
...
rob from to =
  balance from +> \bal ->
    withdrawOp from bal +> depositOp to
...
count x =
  do
    ...
    case lookup x prev of
      Nothing -> modify ((x, 1) :)
      Just a -> put $ map (\(k, v) -> if k == x then (k, a + 1) else (k, v)) prev
